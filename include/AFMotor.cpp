...
/**
 * @brief Construct a new AF_DCMotor object.
 * 
 * @details Initializes a DC motor connected to the motor shield. 
 * It sets up the motor pins and the PWM channel based on the motor number.
 * The constructor also enables the motor controller and initializes the
 * PWM frequency for the specified motor.
 * 
 * @param num The motor number (1-4) that this instance will control.
 * @param freq The PWM frequency to use for controlling the motor speed.
 */
...
AF_Stepper::AF_Stepper(uint16_t steps, uint8_t num)
{
  ...
  if (steppernum == 1)
  {
    latch_state &= ~_BV(M1A) & ~_BV(M1B) &
                   ~_BV(M2A) & ~_BV(M2B); // all motor pins to 0
    MC.latch_tx();
  ...
  {
    latch_state &= ~_BV(M3A) & ~_BV(M3B) &
                   ~_BV(M4A) & ~_BV(M4B); // all motor pins to 0
    MC.latch_tx();

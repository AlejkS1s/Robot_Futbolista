...
/**
 * @brief Initializes the PWM for the motor.
 *
 * This function is called by the MotorCtrl when a motor is enabled.
 * It sets up the ESP32's PWM channel for the motor, with the specified
 * frequency and 8-bit resolution, and attaches the motor's PWM pin to the
 * channel.
 *
 ...
 *
 * This function sets the PWM value for the motor, with the specified
 * 8-bit value.
 *
 * The PWM value is a number between 0 and 255, where 0 is 0% duty cycle and
 * 255 is 100% duty cycle.
 *
 * This function is called by the MotorCtrl when a motor is set to a new speed.
 */
 ...
 * values are FORWARD, BACKWARD, and RELEASE.
 */
...
void StepperMotor::stop()
{
    ...
    controller->updateLatch(latch);
    setSpeed(0);
}

void StepperMotor::oneStep(uint8_t dir, uint8_t style)
{
